...
export function JudgePanel({ questionId, submissions }: JudgePanelProps) {
  ...
  const refreshHistory = useCallback(async () => {
    ...
    setRefreshing(true);
    try {
      const response = await fetch(`/app/api/judge/history?questionId=${encodeURIComponent(questionId)}`, {
        cache: "no-store"
      });
