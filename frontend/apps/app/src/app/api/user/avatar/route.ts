...
export async function POST(request: Request) {
  try {
    ...
    let uploadPayload: ApiEnvelope<UploadResult>;
    try {
      uploadPayload = JSON.parse(uploadText) as ApiEnvelope<UploadResult>;
    } catch {
      return NextResponse.json({ message: uploadText || "头像上传返回格式无效。" }, { status: 502 });
    }
    ...
    return toApiRouteErrorResponse(error, "头像上传失败，请稍后重试。");
  }
}
